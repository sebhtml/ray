...
void CoverageGatherer::work(){
	if(m_distributionOfCoverage.size()==0){
		...
		if(m_parameters->writeKmers()){
			...
			fprintf(kmerFile,"# This file contains the k-mer graph (subgraph of the de Bruijn graph for alphabet={A,T,C,G} and k=%i\n",m_parameters->getWordSize());
		...
		while(iterator.hasNext()){
			...
			#endif
			if(m_parameters->writeKmers()){
				string kmerSequence=idToWord(&key,m_parameters->getWordSize());
				...
				for(int i=0;i<(int)parents.size();i++){
					string printableVersion=idToWord(&(parents[i]),m_parameters->getWordSize());
					if(i!=0)
						fprintf(kmerFile," ");
				...
				for(int i=0;i<(int)children.size();i++){
					string printableVersion=idToWord(&(children[i]),m_parameters->getWordSize());
					if(i!=0)
						fprintf(kmerFile," ");
